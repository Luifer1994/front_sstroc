    ...
    <div class="content-wrapper table-responsive">
      <div class="card">
        <div class="card-body">
          ...
          <form>
            <div class="form-row">
              <div class="form-group col-md-6" v-if="countries.length">
          ...
          <button @click="updatecity()" type="button" class="btn btn-primary mx-2">
            Actualizar
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getcity() {
      const res = await createInstaceAxios.get("city-detail/" + this.$route.params.id);
      this.city.name = res.data.data.name;
      this.city.country_id = res.data.data.country_id;
    },
    async updatecity() {
      ...
      try {
        const res = await createInstaceAxios.put(
          "city-update/" + this.$route.params.id,
          this.city
        );
        if (res.data.res) {
          Toast.fire({
