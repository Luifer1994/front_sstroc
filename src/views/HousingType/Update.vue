<template>
  <div class="main-panel">
    <div class="content-wrapper table-responsive">
      <div class="card">
        <div class="card-body">
          <h4 class="card-title">Editar tipo de vivienda</h4>
          ...
          <button @click="updatehousingType()" type="button" class="btn btn-primary mx-2">
            Actualizar
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async gethousingType() {
      const res = await createInstaceAxios.get(
        "housing-types-detail/" + this.$route.params.id
      );
      this.housingType.name = res.data.data.name;
    },
    async updatehousingType() {
      ...
      try {
        const res = await createInstaceAxios.put(
          "housing-types-update/" + this.$route.params.id,
          this.housingType
        );
        if (res.data.res) {
          Toast.fire({
