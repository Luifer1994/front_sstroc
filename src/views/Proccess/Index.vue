    ...
    <div class="content-wrapper table-responsive">
      <div class="card">
        <div class="card-body">
          ...
          <router-link to="/proccess-register" class="btn btn-primary text-right"
            >Registrar proceso</router-link
          >

          <div v-if="!proccess" class="d-flex justify-content-center mt-4">
            <div
          ...
          <div v-else>
            ...
            <div class="table-responsive">
              <table class="table table-striped">
                ...
                <tbody>
                  <tr v-for="procces in proccess" :key="procces.id">
                    <td>{{ procces.id }}</td>
                    <td>{{ procces.name }}</td>
                    <td>{{ formatDate(procces.created_at) }}</td>
                    <td>
                      <button
                        @click="proccesUpdate(procces.id)"
...
export default {
  ...
  methods: {
    ...
    proccesUpdate(id) {
      this.$router.push({ path: "/proccess-update/" + id });
    },
    async getproccess(limit = null, page = null) {
      if (limit) {
        this.limit = limit;
      }
      if (page) {
    ...
    async delete(id) {
      const res = await createInstaceAxios.delete("process-delete/" + id);
      this.$swal({
        title: "Eliminado!",
