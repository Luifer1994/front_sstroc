<template>
  <div class="main-panel">
    <div class="content-wrapper table-responsive">
      <div class="card">
        <div class="card-body">
          <h4 class="card-title">Fondo de pensión</h4>
          <router-link to="/pensions-fund-register" class="btn btn-primary text-right"
            >Registrar fonde de pensión</router-link
          >

          <div v-if="!pensionFound" class="d-flex justify-content-center mt-4">
            <div
          ...
          <div v-else>
            <div class="table-responsive">
              <table class="table table-striped">
                ...
                <tbody>
                  <tr v-for="found in pensionFound" :key="found.id">
                    <td>{{ found.id }}</td>
                    <td>{{ found.name }}</td>
                    <td>{{ formatDate(found.created_at) }}</td>
                    <td>
                      <button
                        @click="foundUpdate(found.id)"
...
export default {
  ...
  mounted() {
    this.getpensionFound();
  },
  methods: {
    foundUpdate(id) {
      this.$router.push({ path: "/pensions-fund-update/" + id });
    },
    async getpensionFound(limit = null, page = null) {
      if (limit) {
        this.limit = limit;
      }
      if (page) {
      ...
      const res = await createInstaceAxios.get(
        "pension-fund-list-paginated?limit=" + this.limit + "&page=" + this.page
      );
      this.pensionFound = res.data.data.data;
      this.links = res.data.data.links.slice(1, res.data.data.links.length - 1);
    },
    next(num) {
      this.getpensionFound(this.limit, num);
    },
    formatDate(value) {
      moment.locale("es");
      if (value) {
    ...
    async delete(id) {
      const res = await createInstaceAxios.delete("pension-fund-delete/" + id);
      this.$swal({
        title: "Eliminado!",
