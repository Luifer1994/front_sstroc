<template>
  <div class="main-panel">
    <div class="content-wrapper table-responsive">
      <div class="card">
        <div class="card-body">
          <h4 class="card-title">Arls</h4>
          <router-link to="/arls-register" class="btn btn-primary text-right"
            >Registrar Arl</router-link
          >

          <div v-if="!arls" class="d-flex justify-content-center mt-4">
            <div
          ...
          <div v-else>
            <div class="table-responsive">
              <table class="table table-striped">
                ...
                <tbody>
                  <tr v-for="arl in arls" :key="arl.id">
                    <td>{{ arl.id }}</td>
                    <td>{{ arl.name }}</td>
                    <td>{{ formatDate(arl.created_at) }}</td>
                    <td>
                      <button @click="arlUpdate(arl.id)" class="btn btn-warning btn-sm">
                        <img
...
export default {
  ...
  methods: {
    ...
    async getarls(limit = null, page = null) {
      if (limit) {
        this.limit = limit;
      }
      if (page) {
    ...
    async delete(id) {
      const res = await createInstaceAxios.delete("arl-delete/" + id);
      this.$swal({
        title: "Eliminado!",
